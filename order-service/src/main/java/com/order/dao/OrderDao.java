...
public class OrderDao extends ParentDao<OrderRecord, Order, Integer> {
     ...
     *
     * @param ids
     *    {@link List} of {@link Order#getId()} to find
     *
     ...
     * of the given {@link OrderDto#getId()}.
     *
     * @param id
     *    {@link Order#getId()} to find
     *
     ...
     *
     * @param codes
     *    {@link List} of {@link Order#getCode()} to find
     *
     * @return {@link List} of {@link Order}s
     */
    ...
    public Set<OrderDto> fetchPageToOrderDtoByIdWithOrderLineDto(final int page,
        ...
        Table<Record10<Integer, String, Timestamp, Integer, Integer, Short, Double, Short, String, Double>> orderWithLines =
                getOrderWithLinesQuery()
                        .orderBy(OrderTable.ORDER_TABLE.CREATED.desc())
                        .asTable("orderWithLines");

        // Use denseRank function to group the required results (and know the final number of rows to return)
        Table<Record> orderWithLinesAndRank = dsl
                .select(orderWithLines.asterisk()
                        ,denseRank()
                                .over(
                                        orderBy(orderWithLines.field("created").desc())
                                ).as("rank")
                )
                .from(orderWithLines)
                .asTable("orderWithLinesAndRank");

        try (ResultSet rs = dsl
                .select(orderWithLinesAndRank.field("id"),
                        orderWithLinesAndRank.field("code"),
                        orderWithLinesAndRank.field("created"),
                        orderWithLinesAndRank.field("order_lines_id"),
                        orderWithLinesAndRank.field("order_lines_orderId"),
                        orderWithLinesAndRank.field("order_lines_amount"),
                        orderWithLinesAndRank.field("order_lines_cost"),
                        orderWithLinesAndRank.field("order_lines_pizza_id"),
                        orderWithLinesAndRank.field("order_lines_pizza_name"),
                        orderWithLinesAndRank.field("order_lines_pizza_cost")
                )
                .from(orderWithLinesAndRank)
                .where(
                        orderWithLinesAndRank.field("rank")
                                .cast(Integer.TYPE)
                                .between(rankInitial, rankFinal)
                )
                .fetchResultSet()) {
    ...
    private SelectOnConditionStep<Record10<Integer, String, Timestamp, Integer, Integer, Short, Double, Short, String, Double>> getOrderWithLinesQuery() {
        ...
        return dsl
                .select(
                        ...
                        ORDER.CODE,
                        ORDER.CREATED,
                        ORDER_LINE.ID.as("order_lines_id"),
                        ORDER.ID.as("order_lines_orderId"),
                        ORDER_LINE.AMOUNT.as("order_lines_amount"),
                        ORDER_LINE.COST.as("order_lines_cost"),
                        PIZZA.ID.as("order_lines_pizza_id"),
                        PIZZA.NAME.as("order_lines_pizza_name"),
                        PIZZA.COST.as("order_lines_pizza_cost")
                )
                .from(ORDER)
                .leftJoin(ORDER_LINE).on(
                        ORDER_LINE.ORDER_ID.eq(ORDER.ID)
                )
                .join(PIZZA).on(
